...
注意：heartbeat counter 必须按 runId 隔离（使用 Map），不能用模块级全局变量。Worker `concurrency=2` 时多个 run 并发执行，全局变量会导致 run 之间互相干扰。
...
**Worker 兜底 finalizeRun：** Worker catch 块中，无论异常类型，都必须先调用 `finalizeRun(runId, projectId, "failed", message)` 确保 run 不会永久卡在 `running` 状态。这是防止项目被永久阻塞的关键兜底。
...
try {
  ...
  await finalizeRun(runId, projectId, "failed", message).catch(() => {});
...
注：`succeeded -> "running"` 是因为 succeed 后 sandbox 保持活跃，preview 可访问，项目处于"运行中"状态。
...
这两个位置足以覆盖所有关键写入路径。中间写入（sandbox 文件、build log、SSE 事件）即使在 Stop 后发生也无害，因此不需要在每次 DB 写入前都做检查。
...
最坏取消延迟 = 一次 LLM 调用 + 一次 tool 执行 ≈ 30s + 120s = 150s。
...
heartbeat 在 assertRunWritable 内顺带更新（每 N 次检查点更新一次）。

使用 per-run Map 隔离计数，避免并发 run 互相干扰：
...
function resetHeartbeatCounter(runId: string): void {
  heartbeatCounters.delete(runId);
}
```
...
Stop API 只负责让 run 失去写权限，同时更新 project.status 为 stopped。

```text
1. 校验项目归属
2. 查询 active run（queued 或 running）
3. 如果没有 active run，幂等返回成功
4. 事务内条件更新：
   a. 如果 run.status = queued，直接改为 cancelled + project.status = stopped
   b. 如果 run.status = running，改为 cancelling + project.status = stopped
5. 如果原状态是 queued，best-effort 移除 waiting/delayed job
6. 推送 SSE: status_change: stopped
7. 返回 202 Accepted
```
...
throw error; // 其他错误交给 BullMQ retry
```

注意：无论是否重试，Worker catch 块都必须先调用 `finalizeRun` 兜底（见 Section 8.4），确保 run 不会永久卡在 `running`。
